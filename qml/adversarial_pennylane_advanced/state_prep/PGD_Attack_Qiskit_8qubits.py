...
def load_dataset():
    ...
    # Use full 16x16 as in original 8-qubit script
...
class QiskitQuantumFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        # Aggregate grad_inputs back to [768] shape
...
class QML_classifier(torch.nn.Module):
    ...
    def forward(self, x):
        # x is [256]
        inputs_stack = torch.cat([x] * num_reup) # [768]
...
def export_circuit_diagram():
    ...
    # Build a 2-layer version for visualization
    ...
    for l in range(2):
        ...
        for q in range(num_qubits):
            qc.cx(q, (q + r) % num_qubits)
    qc.draw(output='mpl', filename=f'{output_dir}/classifier_circuit.png')
    print(f"Saved circuit diagram to {output_dir}/classifier_circuit.png")
...
visualize_data(perturbed_x.detach().numpy().reshape(-1, 16, 16)[:4], labels_test[:4].numpy(), 
...
plt.close()
print(f"Saved confusion matrix to {output_dir}/confusion_matrix_adversarial.png")
...
plt.close()
print(f"Saved confusion matrix to {output_dir}/confusion_matrix_robust_model.png")
