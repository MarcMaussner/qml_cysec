...
def build_full_circuit(num_qubits, num_layers, num_reup, asp_layers=2):
    ...
    # 2. QDA Noise Injection (ACTIVE Defense Layer)
    # We add a text box or annotation if it were a complex plot, 
    # but here we just show the gates.
...
def main():
    ...
    print("Generating Qiskit NOISY QDA-ASP 4-qubit circuit (Active Defense)...")
    # Draw 2 layers to show the repeating block structure.
    qc_qda = build_full_circuit(num_qubits, num_layers=2, num_reup=3)
    
    fig_qda = qc_qda.draw(output='mpl', style='iqp')
    fig_qda.suptitle("Qiskit Noisy QDA-ASP 4-qubit Circuit (Active Defense)", fontsize=16)
    
    save_path = os.path.join(output_dir, "qiskit_qda_noisy_asp_4q_circuit.png")
