...
output_dir = "pictures_qiskit_qda_noisy_asp_4q"
            ...
            for l in range(num_layers):
                for q in range(num_qubits):
                    for i in range(3):
                        ...
                        # Note: We repeat the QDA noise if present for gradient consistency
...
class QML_classifier(torch.nn.Module):
    ...
    def forward(self, x, idx=None):
        ...
        # Apply QDA - Enabled during BOTH training and test/attack in this script
...
def print_acc(epoch, max_ep=4):
    # Keep QDA ENABLED for evaluation to test active defense
    with torch.no_grad():
...
# Keep QDA ENABLED for Evaluation and Attack
...
def PGD(model, feats, labels, epsilon=0.1, alpha=0.01, num_iter=10):
    ...
    for t in range(num_iter):
        ...
        # Noise is active here, so dL/dx will be randomized
...
print("\n--- Starting Adversarial Retraining (Noisy) ---")
...
with Timer("Adversarial Retraining (Noisy)"):
    ...
    print(f"Post-Retraining Adversarial Accuracy (Noisy QDA): {acc_robust:0.4f}")

visualize_data(skimage.transform.resize(perturbed_x.detach().numpy().reshape(-1, 8, 8), (len(perturbed_x), 8, 8))[:4], 
               labels_test[:4].numpy(), [torch.argmax(p).item() for p in adv_preds_post[:4]], 
               save_name="robustness_evaluation_after_retraining_noisy.png")
...
plt.savefig(f"{output_dir}/confusion_matrix_robust_model_noisy.png")
plt.close()
