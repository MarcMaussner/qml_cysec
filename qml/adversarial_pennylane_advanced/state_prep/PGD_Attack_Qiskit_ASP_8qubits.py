...
output_dir = "pictures_qiskit_asp_8q"
...
class QML_classifier(torch.nn.Module):
    ...
    def forward(self, x, idx=None):
        if idx is not None and idx in self.asp_cache:
            ...
            p, f = asp.fit(x.detach().numpy(), maxiter=100)
        ...
        return QiskitQuantumFunction.apply(inputs_stack, self.weights, self.bias, asp_p)
...
# Training Loop
print(f"Starting Qiskit ASP training loop ({num_qubits} qubits)...")
num_train = feats_train.shape[0]
num_batches = num_train // batch_size

with Timer("QML ASP Model Training"):
