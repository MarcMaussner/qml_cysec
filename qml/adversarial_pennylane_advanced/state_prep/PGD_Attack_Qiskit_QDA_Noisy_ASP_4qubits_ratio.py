...
output_dir = "pictures_qiskit_qda_noisy_asp_4q_ratio"
...
class QiskitQuantumFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs, weights, bias, asp_params=None, qda_std=0.0):
        ...
        qc = QiskitQuantumFunction._build_circuit(inputs_reshaped, w, b, asp_params=asp_params, qda_std=qda_std)
    ...
    @staticmethod
    def _build_circuit(inputs, weights, bias, shifts=None, asp_params=None, qda_std=0.0):
        qc = QuantumCircuit(num_qubits)
        if asp_params is not None:
            asp_qc = asp.get_ansatz(asp_params)
            qc.compose(asp_qc, inplace=True)
            
        # Quantum Data Augmentation (Micro-rotations) - ACTIVE Defense
        if qda_std > 0:
            for q in range(num_qubits):
                qc.rx(np.random.normal(0, qda_std), q)
                qc.ry(np.random.normal(0, qda_std), q)
                qc.rz(np.random.normal(0, qda_std), q)
            ...
            for l in range(num_layers):
                for q in range(num_qubits):
                    for i in range(3):
                        ...
                        circuits.append(QiskitQuantumFunction._build_circuit(inputs_np, weights_np, bias_np, s_p, asp_params=asp_params, qda_std=qda_std))
                        s_m = np.zeros_like(weights_np); s_m[l, q, i] = -shift_val
                        circuits.append(QiskitQuantumFunction._build_circuit(inputs_np, weights_np, bias_np, s_m, asp_params=asp_params, qda_std=qda_std))
...
class QML_classifier(torch.nn.Module):
    def __init__(self, input_dim, output_dim, num_qubits, num_layers, qda_std=0.0):
        super().__init__()
    ...
    def forward(self, x, idx=None):
        ...
        return QiskitQuantumFunction.apply(inputs_stack, self.weights, self.bias, asp_p, self.qda_std)
...
# Enable QDA for training
model = QML_classifier(input_dim, num_classes, num_qubits, num_layers, qda_std=0.05)
...
def print_acc(epoch, max_ep=4):
    with torch.no_grad():
        ...
        print(f"Epoch {epoch}/{max_ep} | Acc val (Benign - NOISY): {acc_test:0.4f}")

# Training Loop
print(f"Starting Qiskit training with ACTIVE QDA Defense ({num_qubits} qubits)...")
num_train = feats_train.shape[0]
num_batches = num_train // batch_size

with Timer("QML training with QDA"):
...
print("\n--- Evaluating with ACTIVE QDA Defense (Test-time Noise Enabled) ---")

with Timer("Benign Evaluation (Noisy)"):
    predictions_test = torch.stack([model(f, idx=1000+i) for i, f in enumerate(feats_test)])
    acc_benign = accuracy(labels_test, predictions_test)
    print(f"Final Benign Accuracy (Noisy): {acc_benign:0.4f}")
    ...
    fig, ax = metric.plot()
    plt.savefig(f"{output_dir}/confusion_matrix_benign_noisy.png")
    plt.close()

visualize_data(X_test[:4], Y_test[:4], [torch.argmax(p).item() for p in predictions_test[:4]], 
               save_name="benign_data_evaluation_noisy.png")
...
print("\n--- Running PGD Attack (Against Noisy model) ---")
attack_samples = 50
with Timer("PGD Attack Generation"):
    ...
    print(f"Adversarial Accuracy (Noisy Defense): {adv_acc:0.4f}")
...
visualize_data(skimage.transform.resize(perturbed_x.detach().numpy().reshape(-1, 8, 8), (len(perturbed_x), 8, 8))[:4], 
               ...
               save_name="adversarial_attack_evaluation_noisy.png")
...
plt.savefig(f"{output_dir}/confusion_matrix_adversarial_noisy.png")
plt.close()

# ## Adversarial Retraining (50/50 Ratio)
print("\n--- Starting Adversarial Retraining (Noisy + 50/50 Ratio) ---")
# Use the ENTIRE training set for adversarial generation (200 samples)
with Timer("Full Adversarial Dataset Generation"):
    all_perturbations = PGD(model, feats_train, labels_train, epsilon=0.1)
    adv_feats_train = feats_train + all_perturbations

# Combine 200 benign + 200 adversarial samples
feats_retrain = torch.cat([feats_train, adv_feats_train])
labels_retrain = torch.cat([labels_train, labels_train])
num_train_retrain = feats_retrain.shape[0]

with Timer("Adversarial Retraining (Noisy + 50/50 Ratio)"):
    retrain_optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    for ep in range(2): # 2 epochs for ratio experiment
        perm = torch.randperm(num_train_retrain)
        for it in range(num_train_retrain // batch_size):
            retrain_optimizer.zero_grad()
            idx = perm[it*batch_size : (it+1)*batch_size]
            batch_feats = feats_retrain[idx]
            batch_labels = labels_retrain[idx]
            # Use distinct indices for cached ASP results (3000+)
            outputs = torch.stack([model(f, idx=3000+i.item()) for i, f in zip(idx, batch_feats)])
            loss = loss_fn(outputs, batch_labels)
            loss.backward()
            retrain_optimizer.step()
        print(f"Retraining Epoch {ep+1}/2 complete.")
    ...
    print(f"Post-Retraining Adversarial Accuracy (Noisy + 50/50 Ratio): {acc_robust:0.4f}")

visualize_data(skimage.transform.resize(perturbed_x.detach().numpy().reshape(-1, 8, 8), (len(perturbed_x), 8, 8))[:4], 
               labels_test[:4].numpy(), [torch.argmax(p).item() for p in adv_preds_post[:4]], 
               save_name="robustness_evaluation_after_retraining_noisy_ratio.png")
...
plt.savefig(f"{output_dir}/confusion_matrix_robust_model_noisy_ratio.png")
plt.close()
