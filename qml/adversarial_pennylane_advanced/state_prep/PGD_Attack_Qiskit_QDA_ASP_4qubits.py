...
class QiskitQuantumFunction(torch.autograd.Function):
    ...
    def _build_circuit(inputs, weights, bias, shifts=None, asp_params=None, qda_std=0.0):
        ...
        # Quantum Data Augmentation (Micro-rotations)
        if qda_std > 0:
...
class QML_classifier(torch.nn.Module):
    ...
    def forward(self, x, idx=None):
        ...
        # Apply QDA only during training or specifically enabled
...
def print_acc(epoch, max_ep=4):
    # Disable QDA for evaluation
    old_qda = model.qda_std
    model.qda_std = 0.0
    with torch.no_grad():
        ...
        print(f"Epoch {epoch}/{max_ep} | Acc val (Benign): {acc_test:0.4f}")
    model.qda_std = old_qda

# Training Loop
print(f"Starting Qiskit training with QDA ({num_qubits} qubits)...")
...
# Disable QDA for Evaluation and Attack
model.qda_std = 0.0
...
with Timer("PGD Attack Generation"):
    ...
    print(f"Adversarial Accuracy: {adv_acc:0.4f}")
...
with Timer("Adversarial Retraining"):
    ...
    print(f"Post-Retraining Adversarial Accuracy (QDA-trained): {acc_robust:0.4f}")
