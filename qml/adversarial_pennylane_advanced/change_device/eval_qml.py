...
def load_dataset():
    ...
    if not os.path.exists(dataset_file):
        # Look in alternative paths to avoid re-downloading
        alt_paths = [
...
# Hyperparameters (defined by original architecture)
input_dim = 8*8
num_classes = 4
num_layers = 16
num_qubits = 4
...
class QML_classifier(torch.nn.Module):
    def __init__(self, input_dim, output_dim, num_qubits, num_layers):
        ...
        self.weights = torch.nn.Parameter(torch.zeros((num_layers, num_qubits, 3)))
        self.bias = torch.nn.Parameter(torch.zeros((num_layers, num_qubits, 3)))
...
def main():
    parser = argparse.ArgumentParser(description="Quantum Machine Learning Model Evaluation & PGD Attacks (Qiskit/PyTorch)")
    parser.add_argument("--params_file", type=str, required=True,
                        help="Path to the saved <backend_name>_training.yaml containing model parameters")
    ...
    print(f"Setting up Qiskit evaluation backend: {backend_name}...")
    ...
    print(f"Loading trained parameters from: {args.params_file}")
    with open(args.params_file, "r") as f:
        saved_data = yaml.safe_load(f)
    
    trained_on_backend = saved_data.get("backend_name", "unknown")
    print(f"Model was originally trained on: {trained_on_backend}")

    # 3. Instantiate model and inject loaded weights
    model = QML_classifier(input_dim, num_classes, num_qubits, num_layers)
    model.weights = torch.nn.Parameter(torch.tensor(saved_data["weights"], dtype=torch.float32))
    model.bias = torch.nn.Parameter(torch.tensor(saved_data["bias"], dtype=torch.float32))

    # 4. Load dataset
    X_train, X_test, Y_train, Y_test = load_dataset()

    # Model evaluation subsets
    ...
    with Timer("Benign Evaluation"):
        ...
        print(f"Final Benign Accuracy on target device {backend_name}: {acc_benign:0.4f}")
    ...
    with Timer("PGD Attack Generation"):
        ...
        print(f"Adversarial Accuracy on device {backend_name} ({attack_samples} samples): {adv_acc:0.4f}")

    visualize_data(perturbed_x.detach().numpy().reshape(-1, 8, 8)[:4], labels_test[:4].numpy(), 
                   [torch.argmax(p).item() for p in adv_preds[:4]], 
                   save_name=f"adversarial_attack_evaluation_{backend_name}.png")
    ...
    plt.savefig(f"pictures_qiskit/confusion_matrix_adversarial_{backend_name}.png")
    plt.close()
    print(f"Saved confusion matrix to pictures_qiskit/confusion_matrix_adversarial_{backend_name}.png")

    # 7. Adversarial Retraining
    ...
    with Timer("Adversarial Retraining"):
        ...
        print(f"Post-Retraining Adversarial Accuracy on target device {backend_name}: {acc_robust:0.4f}")

    visualize_data(perturbed_x.detach().numpy().reshape(-1, 8, 8)[:4], labels_test[:4].numpy(), 
                   [torch.argmax(p).item() for p in adv_preds_post[:4]], 
                   save_name=f"robustness_evaluation_after_retraining_{backend_name}.png")
    ...
    plt.savefig(f"pictures_qiskit/confusion_matrix_robust_model_{backend_name}.png")
    plt.close()
    print(f"Saved confusion matrix to pictures_qiskit/confusion_matrix_robust_model_{backend_name}.png")

    print(f"\n✅ Evaluation & attack complete! Backend: {backend_name}")
