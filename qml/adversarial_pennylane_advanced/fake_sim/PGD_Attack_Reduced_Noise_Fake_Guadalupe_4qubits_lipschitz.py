...
# # Adversarial attacks and robustness for QML - Lipschitz Regularization Experiment
# This script mirrors the golden sample (PGD_Attack_Reduced_Noise_Fake_Guadalupe_4qubits.py)
# exactly, with the addition of Gradient-Based Regularization (Lipschitz) in the training loop.
# The regularization penalizes large gradients with respect to inputs, mathematically bounding
# the model's sensitivity to small perturbations.
...
os.makedirs("pictures_lipschitz", exist_ok=True)
...
def visualize_data(x, y, pred=None, save_name=None):
    ...
    if save_name:
        plt.savefig(os.path.join("pictures_lipschitz", save_name))
        print(f"Saved image to pictures_lipschitz/{save_name}")
    plt.close()
...
class QML_classifier(torch.nn.Module):
    ...
    Key difference from golden sample: a Lipschitz regularization term is added to the
    training loss. The gradient of the loss w.r.t. inputs is computed and its L2 norm
    is penalized (weighted by regularization_rate), mathematically bounding the model's
    Lipschitz constant and thus its sensitivity to adversarial perturbations.
...
epochs = 4
batch_size = 20
# --- LIPSCHITZ: regularization strength ---
...
print(
    f"Starting training loop with Lipschitz Regularization "
    f"({num_qubits} qubits, {num_layers} layers, reg={regularization_rate})..."
)

with Timer("QML Model Training (Lipschitz Regularization)"):
    for ep in range(0, epochs):
        ...
        for it in range(num_batches):
            ...
            # --- LIPSCHITZ: enable input gradients for regularization ---
            feats_train_batch = feats_train[batch_ind[it]].clone().detach().requires_grad_(True)
            labels_train_batch = labels_train[batch_ind[it]]

            outputs = [qml_model(f) for f in feats_train_batch]
            batch_loss = loss(torch.stack(outputs), labels_train_batch)

            # Gradient of loss w.r.t. input samples
            gradients = torch.autograd.grad(
                outputs=batch_loss,
                inputs=feats_train_batch,
                grad_outputs=torch.ones_like(batch_loss),
                create_graph=True,
                retain_graph=True,
                only_inputs=True,
            )[0]

            # L2 norm of the gradient (Lipschitz constraint penalty)
            reg_term = regularization_rate * gradients.pow(2).sum().sqrt()

            total_loss = batch_loss + reg_term
            total_loss.backward()
            # ------------------------------------------------------------
...
preds_max = [torch.argmax(predictions_test[i]).detach().cpu().numpy().item() for i in range(len(predictions_test))]

metric.update(torch.tensor(preds_max), labels_test.cpu())

fig_, ax_ = metric.plot()
plt.savefig(os.path.join("pictures_lipschitz", "confusion_matrix_benign.png"))
print("Saved confusion matrix to pictures_lipschitz/confusion_matrix_benign.png")
plt.close()
...
preds_max = [torch.argmax(adversarial_preds_test[i]).detach().cpu().numpy().item() for i in range(len(adversarial_preds_test))]

metric.update(torch.tensor(preds_max), labels_test.cpu())

fig_, ax_ = metric.plot()
plt.savefig(os.path.join("pictures_lipschitz", "confusion_matrix_adversarial.png"))
print("Saved confusion matrix to pictures_lipschitz/confusion_matrix_adversarial.png")
plt.close()
...
preds_max = [torch.argmax(adversarial_preds_test_increased[i]).detach().cpu().numpy().item() for i in range(len(adversarial_preds_test_increased))]

metric.update(torch.tensor(preds_max), labels_test.cpu())

fig_, ax_ = metric.plot()
plt.savefig(os.path.join("pictures_lipschitz", "confusion_matrix_robust_model.png"))
print("Saved confusion matrix to pictures_lipschitz/confusion_matrix_robust_model.png")
plt.close()
