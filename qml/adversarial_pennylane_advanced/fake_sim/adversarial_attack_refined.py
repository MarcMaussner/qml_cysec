...
class AttackManager:
    @staticmethod
    def generate_pgd_attack(model, loss_fn, feats, labels, epsilon=0.1, alpha=0.01, num_iter=10):
    ...
    @staticmethod
    def generate_fgsm_attack(model, loss_fn, feats, labels, epsilon=0.1):
        delta = torch.zeros_like(feats, requires_grad=True)
        outputs = [model(f) for f in feats + delta]
        l = loss_fn(torch.stack(outputs), labels)
        l.backward()
...
class ExperimentRunner:
    ...
    def run_attack(self, feats, labels, epsilon, alpha, num_iter):
        if self.args.attack_type == "pgd":
            return AttackManager.generate_pgd_attack(
                self.model, self.loss_fn, feats, labels, 
                epsilon=epsilon, alpha=alpha, num_iter=num_iter
            )
        elif self.args.attack_type == "fgsm":
            return AttackManager.generate_fgsm_attack(
                self.model, self.loss_fn, feats, labels, 
                epsilon=epsilon
            )
        else:
            raise ValueError(f"Unknown attack type: {self.args.attack_type}")
    ...
    def run(self):
        ...
        # Attack
        attack_name = self.args.attack_type.upper()
        with Timer(f"{attack_name} Attack Generation"):
            perturbation = self.run_attack(
        ...
        self._evaluate(perturbed_test_feats, test_labels, f"adversarial_{self.args.attack_type}")

        # Adversarial Retraining
        if self.args.retrain_epochs > 0:
            with Timer(f"Adversarial Retraining ({attack_name})"):
                adv_samples_count = min(len(train_feats), 20)
                adv_perturbation = self.run_attack(
                    train_feats[:adv_samples_count], train_labels[:adv_samples_count],
                    epsilon=self.args.epsilon, alpha=self.args.alpha, num_iter=self.args.attack_iter
                )
            ...
            self._evaluate(perturbed_test_feats, test_labels, f"robust_{self.args.attack_type}")
...
def main():
    parser = argparse.ArgumentParser(description="Adversarial Attack on Quantum Machine Learning Model")
    parser.add_argument("--attack_type", type=str, default="pgd", choices=["pgd", "fgsm"], help="Type of adversarial attack")
    ...
    parser.add_argument("--epsilon", type=float, default=0.1, help="Attack epsilon")
