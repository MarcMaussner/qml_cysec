...
# # Adversarial attacks and robustness for QML - Non-Linear Feature Mapping Experiment
# This script mirrors the golden sample (PGD_Attack_Reduced_Noise_Fake_Guadalupe_4qubits.py)
# exactly, with the single addition of a tanh() non-linear activation applied to
# inputs BEFORE encoding into StronglyEntanglingLayers.
...
os.makedirs("pictures_nonlinear", exist_ok=True)
...
def visualize_data(x, y, pred=None, save_name=None):
    ...
    if save_name:
        plt.savefig(os.path.join("pictures_nonlinear", save_name))
        print(f"Saved image to pictures_nonlinear/{save_name}")
    # plt.show()
    plt.close()
...
class QML_classifier(torch.nn.Module):
    ...
    Key difference from golden sample: tanh() non-linear activation is applied
    to the inputs INSIDE the QNode before angle encoding, improving expressivity.
    ...
    def __init__(self, input_dim, output_dim, num_qubits, num_layers):
        ...
        def circuit(inputs, weights, bias):
            # --- NON-LINEAR FEATURE MAPPING (tanh) ---
            # Apply tanh activation before reshaping and encoding.
            # This improves the representation power of the quantum-classical
            # interface by introducing non-linearity at the data encoding stage.
            nonlinear_inputs = torch.tanh(inputs)
            nonlinear_inputs = torch.reshape(nonlinear_inputs, self.weights_shape)
            # -----------------------------------------
            qml.StronglyEntanglingLayers(
                weights=weights * nonlinear_inputs + bias, wires=range(self.num_qubits)
            )
...
# display accuracy and loss after each epoch
...
print(
    f"Starting training loop with Non-Linear Feature Mapping (tanh) "
...
plt.savefig(os.path.join("pictures_nonlinear", "confusion_matrix_benign.png"))
print("Saved confusion matrix to pictures_nonlinear/confusion_matrix_benign.png")
plt.close()


# ## Now attack

# simple implementation of projected gradient descent (PGD) attack
...
fig_, ax_ = metric.plot()
plt.savefig(os.path.join("pictures_nonlinear", "confusion_matrix_adversarial.png"))
print("Saved confusion matrix to pictures_nonlinear/confusion_matrix_adversarial.png")
plt.close()
...
# evaluate - robustness after retraining
adversarial_preds = [qml_model(f) for f in perturbed_x]
adversarial_class_output = [torch.argmax(p) for p in adversarial_preds]
...
# accuracy — use perturbation_test and perturbed_x_test from attack phase
...
# confusion matrix on test set (robust model)
metric = MulticlassConfusionMatrix(num_classes=4)
...
fig_, ax_ = metric.plot()
plt.savefig(os.path.join("pictures_nonlinear", "confusion_matrix_robust_model.png"))
print("Saved confusion matrix to pictures_nonlinear/confusion_matrix_robust_model.png")
plt.close()
